<template>
  ...
  <div @keyup.enter="emitUpdate()" class="w-1/5 relative m-2 h-80 flex-auto flex justify-center items-start flex-col bg-black-light-5">
    ...
    <base-input @update:modelValue="changeN" class="px-8 -mt-8 w-full" :id="note.cne + '_note'" name="Note" :modelValue="note.value" type="number"></base-input>

    <div class="flex-grow"></div>

    <!-- update button -->
    <div class="flex w-full justify-end items-center flex-row p-2">
      <button @click="emitUpdate()" :class="hideClass" class="capitalize rounded-none bg-blue-base px-6 py-2 text-xl font-semibold text-black-base hover:bg-blue-light-1 transition duration-200">
        update
      </button>
    </div>
  </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      hideClass() {
        return this.showUpdate ? 'cursor-pointer ' : 'opacity-0 pointer-events-none';
      },
    },
    methods: {
      emitUpdate() {
