  ...
  <base-body :key="GroupId" class="capitalize overflow-hidden transition duration-200 transform-gpu">
    ...
    <template v-slot:content>
      <div class="w-full mt-28 flex flex-col justify-start items-start space-y-2 px-3 overflow-auto">
        ...
        <div v-if="ShowAddStudent" class="bg-black-light-1 flex justify-between items-center flex-col w-full h-64 relative pb-1 flex-none">
        ...
        <transition-group name="fade-x" mode="out-in" appear>
          <base-student :style="{ 'transition-delay': Math.min(100 * index, 500) + 'ms' }" class="inline-block transition duration-200 transform-gpu" @remove="deleteSt" v-for="(student, index) in group?.students || []" :key="student.cne" :student="student" :gid="GroupId" />
        </transition-group>
      </div>
    </template>
  </base-body>
