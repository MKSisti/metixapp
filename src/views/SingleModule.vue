  ...
  <base-body class="capitalize overflow-hidden pb-2 pt-4">
    ...
    <template v-slot:content>
      <div :class="{'flex-col': tests && tests?.length >= 1}" class="w-full mt-28 flex justify-start items-start space-y-2 px-3 overflow-hidden relative">
        ...
        <!-- test display loop -->
        <div :key="test.id" v-for="test in tests" class="bg-black-light-1 flex justify-between items-center flex-col w-full relative flex-none overflow-hidden">
          <!-- testID internal generated ID (incrementing index or uuid) -->
          <h1 :class="{ collapsedTestID: '[[TEST__COLLAPSED]]' }" class="text-base capitalize absolute top-2 left-2">
            {{ test.id }}
          </h1>
          ...
          <div :class="{ hidden: '[[TEST__COLLAPSED]]' }" class="w-full flex justify-around items-center flex-row flex-wrap pb-10">
            <!-- student note loop -->
            <div :key="Object.entries(note)[0][0]" v-for="note in test.notes" class="w-1/5 h-80 relative m-2 flex-auto flex justify-between items-start flex-col bg-black-light-5 max-w-md">
              ...
              <!-- close btn -->
              <div class="w-full p-2">
                <span class="cursor-pointer hover:bg-red-light-1 text-2xl font-bold overflow-hidden bg-red-base text-black-light-5 w-9 h-9 flex justify-center items-center float-right">
                  <box-icon name="plus" size="cssSize" class="w-full h-full fill-current transform rotate-45 scale-105" v-pre></box-icon>
                </span>
              </div>
              ...
              <base-input class="px-8 -mt-8 w-full" :id="'[[XY00000__ID__NOTE]]'" name="Note" :value="'[[STUDENT__NOTE]]'" type="number" tmp="12"></base-input>

              <!-- update button -->
              <div class="flex w-full justify-end items-center flex-row cursor-pointer p-2">
                <button class="capitalize rounded-none bg-blue-base px-6 py-2 text-xl font-semibold text-black-base hover:bg-blue-light-1 transition duration-200">
                  update
                </button>
              </div>
            </div>
          ...
          <!-- expand button -->
          <div class="flex justify-center items-center flex-row cursor-pointer w-8 h-8 group absolute bottom-0 bg-blue-base hover:bg-blue-light-1">
            <span class="cursor-pointer text-sm flex justify-center items-center w-6 h-6 text-black-base">
